...
solutions = [] # Stores all the solved grids
...
class SudokuGrid():
    ...
    def __init__(self, parent): # Parent is the main window
        self.parent = parent
        self.margin = 20 # margin of the sudoku board
        self.side = 50 # side of each square in the grid
        ...
        self.__initUI() # Creates the other widgets
        self.__solve_grid(grid) ################################################################################################################################################################
    ...
    def __solve_grid(self, grid):
        '''Solves the grid and stores the solutions in solutions list
        
        Displays every iteration of the grid'''
            ...
            for xpos, position in enumerate(row): # Goes through each position in the row
                if position == 0: # Position must be empty
                    for num in range(1,10): # Tries all numbers from 1 to 9
                        if self.__possible(grid, xpos, ypos, num): # Check if the number is a possible
                            grid[ypos][xpos] = num # Puts possible number in empty space
                            self.__solve_grid(grid) # Keeps solving
    ...
    def __possible(self, grid, x, y, n):
    ...
    def __display_solutions(self):
        ...
        print(f'\n-----------------------{len(solutions)} Solutions Found.-----------------------')
        
        for grid in solutions: # Prints out each solution
            print('')
            for row in grid:
                print(row)
